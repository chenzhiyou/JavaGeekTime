...
public class ChatRoomServer {
    ...
    public ChatRoomServer(int port){this.port = port;}
    ...
    class ClientHandler implements Runnable{
        ...
        public void run(){
            System.out.println("处理来自" + socket.getRemoteSocketAddress()+ "的链接");
        ...
        private void handleServerCommand(ChatMessage chatMessage){
            ...
            if (command.equalsIgnoreCase(SERVER_COMMAND_LOGOOF)){
                from.send(new ChatMessage(ADMIN_NAME, chatMessage.getFrom(), BYE));
                from.close();
                System.out.println("用户\"" + chatMessage.getFrom()+ "\"离开聊天室");
            }else if (command.equalsIgnoreCase(SERVER_COMMAND_LOGOOF)){
                String allNames = getAllNames();
                from.send(new ChatMessage(ADMIN_NAME, chatMessage.getFrom()));
            }
        }
